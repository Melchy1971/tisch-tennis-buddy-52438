-- Add 'mitglied' role to the app_role enum
ALTER TYPE app_role ADD VALUE IF NOT EXISTS 'mitglied';

-- Update the handle_new_user function to assign 'mitglied' as default role instead of 'player'
...
BEGIN
  ...
  ELSE
    -- Assign default 'mitglied' role for subsequent users (changed from 'player')
    INSERT INTO public.user_roles (user_id, role)
