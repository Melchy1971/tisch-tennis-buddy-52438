...
-- Create storage policies for club logos (drop existing ones first)
DROP POLICY IF EXISTS "Club logos are publicly accessible" ON storage.objects;
DROP POLICY IF EXISTS "Admin and vorstand can upload club logos" ON storage.objects;
DROP POLICY IF EXISTS "Admin and vorstand can update club logos" ON storage.objects;
DROP POLICY IF EXISTS "Admin and vorstand can delete club logos" ON storage.objects;
